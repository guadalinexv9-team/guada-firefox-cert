...
const gCCKBundle = gBundleService.createBundle("chrome://cck-guada/content/cck.properties");
  ...
  observe: function observe(subject, topic, data) {
    switch (topic) {
      ...
      case "final-ui-startup":
        init();
        if (Cc["@mozilla.org/extensions/manager;1"]) {
          var location = em.getInstallLocation("guada@ext.guadalinex.tld").getItemLocation("guada@ext.guadalinex.tld");
          installXPIs(location);
        } else {
          Components.utils.import("resource://gre/modules/AddonManager.jsm");
          AddonManager.getAddonByID("guada@ext.guadalinex.tld", function(addon) {
            if (addon) {
...
//        livemarks = Cc["@mozilla.org/browser/livemark-service;2"].getService(Ci.nsILivemarkService);
      ...
      case "em-action-requested":
        ...
        if (subject.id == "guada@ext.guadalinex.tld") {
          switch (data) {
...
function init() {
  ...
  if (gPrefBranch.prefHasUserValue("extensions.guada@ext.guadalinex.tld.version")) {
    installedVersion = gPrefBranch.getCharPref("extensions.guada@ext.guadalinex.tld.version");
  }

  if (!gPrefBranch.prefHasUserValue("extensions.guada@ext.guadalinex.tld.install-event-fired")) {
    firstrun = true;
    gPrefBranch.setBoolPref("extensions.guada@ext.guadalinex.tld.install-event-fired", true);
  }
  ...
  if (firstrun || extVersion != installedVersion)
  {
    ...
    gPrefBranch.setCharPref("extensions.guada@ext.guadalinex.tld.version", extVersion);
  }
}
...
function uninit() {
  if (gPrefBranch.prefHasUserValue("extensions.guada@ext.guadalinex.tld.install-event-fired")) {
    gPrefBranch.clearUserPref("extensions.guada@ext.guadalinex.tld.install-event-fired");
  }

  if (gPrefBranch.prefHasUserValue("extensions.guada@ext.guadalinex.tld.version")) {
    gPrefBranch.clearUserPref("extensions.guada@ext.guadalinex.tld.version");
  }
...
function addCertificate(CertName, CertTrust) {
  ...
  var channel = gIOService.newChannel("chrome://cck-guada/content/" + CertName, null, null);
...
let listener = {
  onUninstalling: function(addon) {
    if (addon.id == "guada@ext.guadalinex.tld") {
      CCK.beingUninstalled = true;
    }
  },
